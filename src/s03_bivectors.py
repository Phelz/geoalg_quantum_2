...
# class GeometricAlgebra(ThreeDScene):
    def construct(self):
        ...
        title = MarkupText(
            "Geometric Algebra", color=BLUE_D, font_size=dfs.TITLE_FONTSIZE
        ).to_edge(UP)
        self.add(title)
        ...
        dot_name = MathTex(
            "Scalar", color=WHITE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
        ).next_to(dot, UP)
        ...
        vector_name = MathTex(
            "Vector", color=WHITE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
        ).next_to(vector, UP)
        self.play(
            Create(vector),
        ...
        bivector_1_name = MathTex(
            "\\vec{u}", color=WHITE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
        ...
        bivector_2_name = MathTex(
            "\\vec{v}", color=WHITE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
        ...
        bivector_title = (
            MathTex("Bivector", color=WHITE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3))
        ...
        rotation_symbol = MathTex(
            "\\circlearrowright", color=WHITE, font_size=int(5 * dfs.TITLE_FONTSIZE / 3)
        )
        ...
        # *______________________________________________________________________
        self.next_section("Bivector Orientation", skip_animations=False)
        # *______________________________________________________________________
        ...
        bivector_1_name_redone = MathTex(
            "\\mathbf{u}", color=RED, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
        ).move_to(bivector_1_name.get_center())
        bivector_2_name_redone = MathTex(
            "\\mathbf{v}", color=BLUE, font_size=int(3 * dfs.TITLE_FONTSIZE / 3)
